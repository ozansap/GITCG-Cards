...
<div class="w-full rounded-3xl border-2 border-color_text p-2 text-sm text-color_text shadow-10 shadow-white/20">
	<div class="flex items-center justify-between gap-2 {expanded && 'mb-8'}">
		<div class="flex flex-1">
			<button on:click={() => (cardType = 'character')} class="min-w-[5rem] flex-1 rounded-l-full border-2 border-r-0 border-color_text py-1 text-center transition {cardType === 'character' && 'border-r-2 border-white bg-white/20 text-white'}">
				<p>Character</p>
			</button>
			<button on:click={() => (cardType = 'action')} class="min-w-[5rem] flex-1 rounded-r-full border-2 border-l-0 border-color_text py-1 text-center transition {cardType === 'action' && 'border-l-2 border-white bg-white/20 text-white'}">
				<p>Action</p>
			</button>
		</div>
				...
				<fieldset class="relative mt-6 rounded-full border-2 border-color_text text-xs">
					...
					<div class="grid {Filters.styles.category[category.value].grid} px-1 text-xs">
						{#each category.options as option, k}
							<div class="px-1 py-2">
								<button on:click={() => (option.active = !option.active)} class="flex h-full w-full items-center justify-center rounded-full border-2 border-color_text p-1 text-center {option.active && Filters.styles.option[option.value].active}">
									{#if menuType === 'character'}
										<img src="/ui/elements/{option.name.toLowerCase()}.png" alt={option.name} class="flex-1 grayscale {option.active && 'grayscale-0'}" />
									{:else}
										<p>{option.name}</p>
									{/if}
								</button>
